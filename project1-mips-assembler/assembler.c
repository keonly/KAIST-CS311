...
struct {
    ...
    = { { 0, 0x21 },   { 1, 0x24 },  { 2, 0x08 },  { 3, 0x27 },  { 4, 0x25 },
        { 5, 0x2B },   { 6, 0x00 },  { 7, 0x02 },  { 8, 0x23 },  { 9, 0x09 },
        { 10, 0x0C },  { 11, 0x04 }, { 12, 0x05 }, { 13, 0x0F }, { 14, 0x23 },
        { 15, 0x0D },  { 16, 0x0B }, { 17, 0x2B }, { 18, 0x02 }, { 19, 0x03 },
        { NULL, NULL } };
...
instr_to_bin_string (char *input, int program_counter)
{
    ...
    case I_FORMAT: {
        ...
        if (instruction == 11 || instruction == 12) {
            ...
            i_code.im
                = (((int) (found_label->data - (program_counter + 4)) >> 2)) & 0xFFFF;
        ...
        } else if (instruction == 9 || instruction == 10 || instruction == 15
                   || instruction == 16) {
...
{
    return int_to_bin_string ((code.op << 26) + (code.rs << 21) + (code.rt << 16)
                              + (code.rd << 11) + (code.sh << 6) + code.fn);
}
